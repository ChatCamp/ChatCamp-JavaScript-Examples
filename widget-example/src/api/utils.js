...
import { getLocalToken, getAccessToken } from 'api/AuthSvc'

function requestWrapper (method) {
  return async function (url, data = null, params = {}) {
    let csrfTokenResult = await getCsrfToken('/api/3.0/csrf');
    let csrfToken = csrfTokenResult.data.value;
    if (method === 'GET') {
      // is it a GET?
    ...
    if (url.match(/^https?:\/\//gi) > -1) {
      ...
      if(access_token) {
        defaults.headers['X-Access-Token'] = access_token
      }
...
async function getCsrfToken (url, data = null, params = {}) {
...
export function getAppIdFromURL() {
  ...
  if(parentUrl.toLowerCase().indexOf(".iflychat") > -1 || parentUrl.toLowerCase().indexOf(".localhost") > -1) {
    parentUrl = parentUrl.replace(/(^\w+:|^)\/\//, '');
    let endIndex = parentUrl.indexOf(".iflychat") > -1 ? parentUrl.indexOf(".iflychat") : parentUrl.indexOf(".localhost");
     return parentUrl.substring(0,endIndex)
  } else {
    return null
  }
}

...
class WindowContent extends Component {
  ...
  render () {
    ...
    this.props.groupChannels.getIn([this.props.id, 'messages'], false).map((message, key) => {
      ...
      let classes = "window-message window-message-operator"
      let operator = true
      // let source =  sourceURL + "icons8-customer-80.png"
      if(role === "operator"){
        console.log("operator")
        messageAvatar = <Comment.Avatar className= "chatcamp-operator-avatar" src={source} />
      }
      if(role !== "operator"){
        console.log("user")
        classes = "window-message window-message-user"
        messageAvatar = false
        operator = false
      }
      ...
      if(text!=null) messages.push(
        <Comment.Group key={"window-message-" + message.get('id')} className={classes + " window-message-" + message.get('id')}>
          <Comment>
            {!messageClubbing.info && operator && messageAvatar}
            {messageClubbing.info && operator && <Comment.Avatar as={() => <AvatarWrapper color="#fff" className="avatar" name= "test"/>} />}
            <Comment.Content>
              {/*  message author*/}
              {operator && !messageClubbing.info && <Comment.Author>{message.getIn(['user', 'displayName'])}</Comment.Author>}
              ...
              <Comment.Text>
                ...
                {false && <ReadReceipt groupChannelId ={this.props.id} message={message} />}
                {customType==="action_link" && <MessageActionCard product={JSON.parse(metadata.product)} />}
              </Comment.Text>
              {/* <Comment.Actions>
                {!messageClubbing.info && <Comment.Action>
                  <div>{UtilityTime.getTime('1', message.get('insertedAt')*1000)}</div>
                </Comment.Action>}
              </Comment.Actions> */}
            </Comment.Content>
